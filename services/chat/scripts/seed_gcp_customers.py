...
class CustomerSeeder:
    ...
    def load_customer_files(self, data_dir: Path) -> List[Dict[str, Any]]:
        """Load all customer JSON files from the specified directory."""
        customers = []

        # Find all customer JSON files
        customer_files = glob.glob(str(data_dir / "customer_info_*.json"))

        if not customer_files:
            logger.warning(f"No customer files found in {data_dir}")
            return customers

        logger.info(f"Found {len(customer_files)} customer files")

        for file_path in sorted(customer_files):
            try:
                with open(file_path, 'r', encoding='utf-8') as file:
                    ...
                    if self.validate_customer_data(customer_data):
                        ...
                        logger.debug(f"Loaded customer {customer_data['id']} from {file_path}")
                    else:
                        logger.error(f"Invalid customer data in {file_path}")
    ...
    def seed_customers(self, data_dir: Path, force_update: bool = False) -> bool:
        """Main method to seed customers into Firestore."""
        try:
            # Load customer data
            customers = self.load_customer_files(data_dir)
...
def main():
    ...
    # Path to customer data directory
    script_dir = Path(__file__).parent
    data_dir = script_dir.parent / "data" / "customer_info"

    if not data_dir.exists():
        logger.error(f"Customer data directory not found at: {data_dir}")
        sys.exit(1)
    ...
    try:
        ...
        success = seeder.seed_customers(data_dir, force_update)
